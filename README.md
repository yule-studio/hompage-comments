...
그래서 매칭이 있는 페이지는 **이미지로 다시 그리고 콘텐츠 스트림을 통째로 교체한다.** 지워진 것이 지워진 이유는 덮어서가 아니라 **그 글자를 그리던 명령이 없어져서**다.
...
0
$ pdftotext -f 1 -l 1 발급본.pdf - | wc -c    # 마스킹된 페이지
0        # 텍스트 레이어 자체가 없다
```

**매칭이 없는 페이지는 건드리지 않는다.** 숨길 것이 없으니 원본 그대로 지나가고, 덕분에 벡터 텍스트와 선택 가능성이 유지된다. 발급본에서 텍스트가 안 잡히는 건 전화번호가 있는 페이지 하나뿐이다.

### 흐릿하게 가리되, 흐리게 해서 가리지는 않는다

가림막은 반투명 + 블러다. 다만 **진짜 글자를 블러 처리하는 것이 아니다.**

전화번호는 알려진 폰트로 그려진 8자리 숫자다. 후보를 전부 렌더링해서 같은 방식으로 뭉갠 뒤 대조하면 원본을 복원할 수 있을 만큼 경우의 수가 적다. 블러·모자이크 마스킹을 되돌리는 공개 도구가 실제로 이 방식으로 동작한다.

그래서 순서가 중요하다.

1. **지운다** — 해당 영역을 옆에서 샘플링한 배경색으로 덮어버린다. 이 시점에 글자 정보는 사라진다
2. **가린다** — 둥근 반투명 패널을 그린다
3. **부드럽게** — 블러를 건다. 이때 영역 안에는 합성한 색밖에 없으므로 새어나갈 것이 없다

블러는 **보기 좋으라고 있는 것이지 보호 장치가 아니다.** `RESUME_MASK_OPACITY` 를 1.0 으로 올리든 0.3 으로 낮추든 안전성은 달라지지 않는다.
...
| `RESUME_MASK_COLOR` | `#9AA0A6` | 가림막 색 |
| `RESUME_MASK_OPACITY` | `0.72` | 가림막 불투명도. **외형만 바뀐다** — 아래 픽셀은 이미 지워져 있다 |
...
- **속도** — 발급 1회는 매칭이 있는 **페이지만** 150 DPI 로 렌더링한다. 전 페이지를 렌더링하던 초기 구현은 현재 이력서에서 40초가 걸렸고(한 페이지가 혼자 12초), 페이지를 골라 렌더링하면서 2초대로 내려왔다. 페이지 수가 아니라 **마스킹 대상이 있는 페이지 수**가 비용이다.
- **파일 크기** — 원본 4.4MB → 발급본 3.8MB. 전 페이지를 래스터화하던 때(1.8MB)보다는 크다. 원본 페이지를 그대로 통과시키니 폰트·이미지가 함께 남기 때문이고, 텍스트 선택이 되는 쪽이 이력서에는 낫다고 봤다.
